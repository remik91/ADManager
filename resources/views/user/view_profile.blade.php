                ...
                <div class="card-body d-flex align-items-center justify-content-between gap-3">
                    <div class="d-flex align-items-center gap-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center"
                            style="width:64px;height:64px;background:#558f98;color:#fff;font-size:22px;font-weight:600;">
                            {{ $initial }}
                        </div>
                        <div>
                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                <h5 class="mb-0">{{ $user->displayName[0] ?? $user->cn[0] }}</h5>
                                @if ($user->isEnabled())
                                    <span class="badge bg-success">Activé</span>
                                @else
                                    <span class="badge bg-warning text-dark">Désactivé</span>
                                @endif
                                <span class="badge bg-secondary">OU : {{ $user->getParentName() }}</span>
                                <span class="badge bg-light text-dark">UID : {{ $user->sAMAccountName[0] }}</span>
                            </div>
                            <div class="text-muted small mt-1">
                                Dernière connexion : {{ $user->lastlogon }} &middot; Créé le : {{ $user->whencreated }}
                            </div>
                            <div class="small">
                                <a href="mailto:{{ $user->mail[0] }}">{{ $user->mail[0] }}</a>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="confirmDeleteLabel" class="modal-title">Supprimer le compte</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    Cette action est irréversible. Confirmer la suppression de <strong>{{ $user->displayName[0] }}</strong>
                    ?
                </div>
                <div class="modal-footer">
                    <button class="btn btn-link" data-bs-dismiss="modal">Annuler</button>
                    <a href="{{ route('user.remove', $user->getDn()) }}" class="btn btn-danger">Supprimer</a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <div class="tab-pane fade" id="pills-ldapacad" role="tabpanel"
                                ...
                                <div class="d-flex flex-column align-items-center text-center mb-3">
                                    <div class="btn-group" role="group">
                                        <button class="btn btn-outline-primary" type="button" id="btncheckldap"
                                            data-id="{{ $user->sAMAccountName[0] }}">
                                            <i class="fas fa-user-check me-1"></i> Afficher depuis LDAP
                                        </button>
                                        <button class="btn btn-outline-secondary" type="button" id="btnSyncOpen"
                                            disabled data-bs-toggle="modal" data-bs-target="#syncModal">
                                            <i class="fa-solid fa-people-pulling me-1"></i> Synchroniser vers AD
                                        </button>
                                    </div>
                                    <div class="form-text mt-2">Lecture seule côté LDAP. Utilise « Synchroniser » pour
                                        mettre à jour la fiche AD.</div>
                                </div>

                                <div id="resultats_checkldap">
                                    <div class="card">
                                        <div class="card-body">
                                ...
                                <div class="modal fade" id="syncModal" tabindex="-1" aria-labelledby="syncModalLabel"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="POST" action="{{ route('user.resynchro') }}" id="formSync">
                                                @csrf
                                                <input type="hidden" name="ad_dn" id="sync_ad_dn" value="">
                                                <input type="hidden" name="ldap_dn" id="sync_ldap_dn" value="">

                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="syncModalLabel"><i
                                                            class="fa-solid fa-people-pulling me-2"></i>Synchroniser depuis
                                                        LDAP</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p class="mb-2">Cette action va <strong>importer</strong> les
                                                        informations affichées depuis le LDAP académique vers la fiche
                                                        <strong>Active Directory</strong> de l’utilisateur.
                                                    </p>
                                                    <ul class="small text-muted mb-0">
                                                        <li>Certains champs AD (ex. UPN, groupes, mots de passe) ne sont
                                                            <em>pas</em> impactés.
                                                        </li>
                                                        <li>Les valeurs vides ne remplacent pas celles d’AD.</li>
                                                    </ul>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-link"
                                                        data-bs-dismiss="modal">Annuler</button>
                                                    <button type="submit" class="btn btn-primary">Synchroniser
                                                        maintenant</button>
                                                </div>
                                            </form>
                                        </div>
...
@section('scriptjs')
    <script type="module">
        ...
        ////CHECK LDAP ET RESYNCHRO
        const $btnCheck = $('#btncheckldap');
        const $btnSync = $('#btnSyncOpen');
        const $panel = $('#resultats_checkldap');
      ...
      <div class="card"><div class="card-body">
        ...
        async function fetchLdap() {
            const uid = $btnCheck.data('id');
            if (!uid) {
                toastr?.error('UID manquant');
                return;
            }
            ...
            $.ajax({
                ...
                .done((html) => {
                    $panel.html(html);
                    // Récup des DN pour le bouton Sync
                    const adDn = $('#ldapcard').data('ad-dn');
                    const ldapDn = $('#ldapcard').data('ldap-dn');
                    if (adDn && ldapDn) {
                        $('#sync_ad_dn').val(adDn);
                        $('#sync_ldap_dn').val(ldapDn);
                ...
                .fail((xhr) => {
                    let msg = 'Erreur lors de la requête LDAP.';
                    if (xhr?.responseText) msg = xhr.responseText;
                    $panel.html(`<div class="alert alert-danger">${msg}</div>`);
                    $btnSync.prop('disabled', true);
                })
                .always(() => {
        ...
        // Chargement auto au premier affichage de l’onglet
        document.addEventListener('shown.bs.tab', (e) => {
            if (e.target?.id === 'pills-ldapacad-tab') {
